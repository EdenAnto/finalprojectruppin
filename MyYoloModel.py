...
import random, hashlib, os, torch
import sys
...
class MyYolo:
    def __init__(self, taskDir, fileName,weightsPath):
        splittedName = fileName.split('.')
        self.model = YOLOv10(weightsPath)
        self.videoName=fileName.split('_')[0]
        self.basePath=taskDir
        self.fileName = fileName
        self.format = splittedName[-1]
        self.nameNdate = splittedName[0]
        self.inputDir = os.path.join(taskDir, "input",fileName)
        self.outDir =  os.path.join(taskDir, "output")
        self.log =setup_logger('taskLog', taskDir)
        self.outFileDir =  os.path.join(self.outDir, f"tagged_{fileName}")
        self.inputFrames = os.path.join(taskDir, "frames","frames")
        self.taggedFrame = os.path.join(taskDir, "frames","taggedframes")
        self.frameCount = -1
        self.prompt={'current': ''}
        self.promptSet=set()
        self.nextStep= [-1]
    ...
    def start(self):
        ...
        try:
            ...
            raise Exception(f"writing statistics: {e}")
    ...
    def framesToVideo(self,fps=24):
        ...
        with warnings.catch_warnings():
            ...
            with ImageSequenceClip(frame_paths, fps=fps) as clip:
                clip.write_videofile(output_video_path, codec='libx264')
    ...
    def setStage(self, stage):
        if stage == 1:
            ...
            try:
                self.log.info('Here is all data of the video:')
                self.log.info(self.generateDbData())
            except Exception as e:
                print(e)
...
def clear_or_create_directory(directory_path):
    try:
        ...
        raise Exception(f"Error while clearing or creating the directory: {e}")

if __name__ == '__main__':
    path = os.path.dirname(os.path.abspath(__file__))
    directories_to_clear = [os.path.join(path, "MediaTesting", "frames", "frames"),os.path.join(path, "MediaTesting", "frames", "taggedframes"),os.path.join(path, "MediaTesting", "output")]

    video_path = os.path.join(path, "MediaTesting")
    weights_path = os.path.join(path, "Configuration", 'weights', 'yolov10s.pt') 

    try:
        for dir in directories_to_clear:
            clear_or_create_directory(dir)

        task = MyYolo(video_path, "Accident_19-12-24_12_40.mp4",weights_path)
